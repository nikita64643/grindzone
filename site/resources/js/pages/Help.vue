...
const props = withDefaults(
    ...
    {
        intro: () => ({ headline: 'Игровая поддержка', paragraphs: [] }),
        chapters: () => [],
        sidebar: () => ({}),
        initialServerStatus: () => ({}),
    }
...
<template>
    <Head title="Вопросы и ответы — GrindZone" />
        ...
        <div class="mx-auto max-w-6xl px-4 py-8 sm:py-12">
            <div class="mb-8 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">Ответы на частые вопросы</p>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-8 lg:flex-row lg:gap-10">
                <div class="min-w-0 flex-1">
                    ...
                    <div class="space-y-3">
                        ...
                        >
                            <CollapsibleTrigger as-child class="block w-full">
                                <button
                                    type="button"
                                    class="group flex w-full items-center justify-start gap-3 rounded-t-2xl px-6 py-4 text-left transition focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-inset hover:bg-zinc-50 dark:hover:bg-zinc-800/50 sm:px-8"
                                >
                            ...
                            <CollapsibleContent>
                                <ul class="grid gap-4 p-6 sm:grid-cols-1 sm:px-8 sm:py-6 lg:grid-cols-2">
                                    <li
                                        v-for="(item, idx) in chapter.items"
                                        :key="idx"
                                        class="rounded-xl border border-zinc-100 bg-zinc-50/50 p-4 dark:border-zinc-800 dark:bg-zinc-900/30"
                                    >
                                        <h4 class="mb-2 font-semibold text-zinc-900 dark:text-white">{{ item.question }}</h4>
